...
`resources/js/etabs/` (mounted from `main.js` / `App.vue`) is the largest module: a Vue 3 CAD/BIM-style viewer and modeler (`resources/js/cad/**` — 2D canvas, 3D via Babylon.js, DXF/DWG import via `@mlightcad/*`, grids, selection, dialogs, model I/O) that talks to the seismic/structural analysis API and to `CadModelController` for autosave and named-model CRUD (`etabs/model/autosave`, `etabs/models*`).
