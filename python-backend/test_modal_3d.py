...
X = [0.0, 6.0, 12.0, 18.0]  # ejes en X (m)
Y = [0.0, 6.0, 12.0, 18.0]  # ejes en Y (m)
...
print(
    f"  Ejemplo  nodo {nid_of[(0,0,0)]} = (0, 0, 0)   ·   nodo {nid_of[(NX-1,NY-1,NZ-1)]} = ({X[-1]}, {Y[-1]}, {Z[-1]})"
)
...
print(
    f"  {len(base)} nodos de la base (z=0) empotrados: UX UY UZ RX RY RZ = 1 1 1 1 1 1"
)
...
print(
    f"  Viga    30x50: A={viga['A']}  Iy={viga['Iy']}  Iz={viga['Iz']}  J={viga['J']}"
)
...
print(
    "  Transf 1 (columnas + vigas-X): vecxz=(0,1,0)  → Iz fuerte resiste X / vigas paradas"
)
...
for k in range(1, NZ):
    for j in range(NY - 1):
        for i in range(NX):
            ...
            n_by += 1
print(
    f"  Columnas: {n_col}   Vigas-X: {n_bx}   Vigas-Y: {n_by}   ·   Total: {eid-1} elementos"
)
...
for k in range(1, NZ):
    ...
    print(
        f"  Piso {k} (z={Z[k]:>4.0f}m): {masa_piso[k]/1000:>6.1f} t  →  {m_nodo:>8.1f} kg/nodo en UX,UY"
    )
...
for solver_args in [
    ...
    try:
        print(f"  Probando solver eigen: {solver_args if solver_args else 'default'}")
        resultado = ops.eigen(*solver_args, NUM_MODOS)

        if resultado and len(resultado) > 0:
            lam = resultado
            print(f"  ✅ Solver usado: {solver_args if solver_args else 'default'}")
            break

    except Exception as e:
        errores_solver.append(f"{solver_args if solver_args else 'default'}: {e}")
        print(f"  ⚠️ Falló solver {solver_args if solver_args else 'default'}: {e}")

if lam is None:
    raise RuntimeError(
        "No se pudo ejecutar el análisis modal con ningún solver. "
        f"Errores: {errores_solver}"
    )

print(f"  {'Modo':>4} | {'T [s]':>8} | {'f [Hz]':>8} | {'ω [rad/s]':>10}")

for i, l in enumerate(lam, 1):
    l_real = float(l.real) if hasattr(l, "real") else float(l)
    w = math.sqrt(abs(l_real)) if abs(l_real) > 1e-12 else 0.0
    f = w / (2 * math.pi) if w > 0 else 0.0
    T = 1 / f if f > 0 else 0.0
    print(f"  {i:>4} | {T:>8.4f} | {f:>8.4f} | {w:>10.4f}")
...
try:
    ...
    mx, my, rz = (
        mp.get("partiMassRatiosMX", []),
        mp.get("partiMassRatiosMY", []),
        mp.get("partiMassRatiosRMZ", []),
    )
    cmx, cmy = mp.get("partiMassRatiosCumuMX", []), mp.get("partiMassRatiosCumuMY", [])
    print(
        f"  {'Modo':>4} | {'MX %':>7} {'MY %':>7} {'RZ(tors)%':>10} | {'sumaMX%':>8} {'sumaMY%':>8}"
    )
    for i in range(len(mx)):
        print(
            f"  {i+1:>4} | {mx[i]:>7.2f} {my[i]:>7.2f} {rz[i]:>10.2f} | {cmx[i]:>8.2f} {cmy[i]:>8.2f}"
        )
except Exception as e:
...
try:
    ...
    print(
        "  Abriendo 2 ventanas: el edificio y la forma del Modo 1. Cierra para terminar."
    )
    plt.show()
except ImportError:
