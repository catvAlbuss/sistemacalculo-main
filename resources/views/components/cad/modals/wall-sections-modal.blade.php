{{-- resources/views/components/cad/modals/wall-sections-modal.blade.php --}}
{{-- Réplica de slab-sections-modal.blade.php para Muros: lista de Wall Properties
     (paso 1) + editor Wall Property Data (paso 2). El espesor + material definen
     tanto el peso propio como la rigidez del panel shell en el motor sísmico. --}}
<div x-data="wallSectionsModal()"
    x-init="init()"
    x-show="open"
    x-cloak
    ...
    <div x-show="view === 'list'" class="bg-gray-800 rounded-lg shadow-2xl w-[520px] border border-gray-700">
        <div class="px-4 py-3 border-b border-gray-700 bg-gray-900">
            <h3 class="text-lg font-semibold text-white">Propiedades de Muro (Wall Sections)</h3>
        ...
        <div class="p-4">
            <div class="grid grid-cols-[1fr_auto] gap-3">
                <div class="border border-gray-700 rounded">
                    <div class="px-2 py-1 text-xs text-gray-400 border-b border-gray-700">Propiedad de Muro</div>
    ...
    {{-- ══════════ PASO 2: EDITOR (Wall Property Data) ══════════ --}}
    <div x-show="view === 'editor'" class="bg-gray-800 rounded-lg shadow-2xl w-[560px] max-h-[92vh] overflow-hidden border border-gray-700">
        <div class="px-4 py-3 border-b border-gray-700 bg-gray-900">
            <h3 class="text-lg font-semibold text-white">Datos de Propiedad de Muro (Wall Property Data)</h3>
        ...
        <div class="p-4 overflow-y-auto max-h-[78vh] space-y-4">
            ...
            <div class="border border-gray-700 rounded p-3">
                ...
                <div class="space-y-2 text-sm">
                    ...
                    <div class="flex items-center gap-2">
                        <span class="text-gray-300 w-40">Material de Muro</span>
            ...
            <div class="border border-gray-700 rounded p-3">
                ...
                <div class="mt-3 pt-2 border-t border-gray-700 text-xs text-emerald-300">
                    ...
                    (espesor × densidad del material).
                </div>
            </div>

            <div class="text-[11px] text-gray-500 leading-snug">
                El espesor define tanto el <b>peso propio</b> como la <b>rigidez lateral</b> del panel de
                muro (se malla con elementos shell en el análisis sísmico) — no es solo decorativo como en
                una losa. El "Tipo de Modelado" es informativo por ahora (el motor usa una formulación
                shell fija equivalente a Shell-Thin).
            </div>
        </div>
...
<script>
    function wallSectionsModal() {
        return {
            ...
            editing: {
                name: '', material: 'CONC', modelingType: 'Shell-Thin', thickness: 150, color: '#78716c',
            },
            ...
            init() {
                this.loadSections();
                window.addEventListener('open-wall-sections-modal', () => this.openModal());
                window.addEventListener('cad-units-changed', () => { this.unitsVersion++; });
            },

            loadSections() {
                const cs = window.cadSystem || {};
                let list = Array.isArray(cs.wallSections) ? cs.wallSections : null;
                if (!list || !list.length) {
                    list = [
                        { name: 'Muro e=0.15', material: 'CONC', modelingType: 'Shell-Thin', thickness: 150, color: '#78716c' },
                        { name: 'Muro e=0.20', material: 'CONC', modelingType: 'Shell-Thin', thickness: 200, color: '#57534e' },
                    ];
                }
            ...
            defaultEditing(name) {
                return { name: name || '', material: this.availableMaterials[0] || 'CONC', modelingType: 'Shell-Thin', thickness: 150, color: '#78716c' };
            },

            addNew() {
                let name = 'Muro' + (this.sections.length + 1);
            ...
            saveAll() {
                if (window.cadSystem) {
                    ...
                    window.cadSystem.wallSections = JSON.parse(JSON.stringify(sections));
                    this.showToastMessage('✅ Propiedades de muro guardadas (' + this.sections.length + ')', 'success');
                    window.cadSystem.showMessage?.('Propiedades de muro guardadas: ' + this.sections.length + '.');
                }
                this.close();
            },
        };
