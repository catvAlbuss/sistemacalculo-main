{{-- resources/views/components/cad/modals/joint-diaphragms-modal.blade.php
     Assign ▸ Joint ▸ Diaphragms, estilo ETABS "Joint Assignment - Diaphragm".
     Migración Swal→Blade (fase JS parte b): el mixin valida selección, arma la
     lista y dispara 'open-joint-diaphragms-modal'; aplica vía
     applyJointDiaphragmFromModal(). --}}
<div x-data="jointDiaphragmsModal()"
     x-show="open"
     x-cloak
    ...
    <div class="bg-gray-800 rounded-lg shadow-xl border border-gray-700" style="width:340px; max-width:95vw">
        ...
        <div class="flex items-center justify-between px-4 py-2 border-b border-gray-700 bg-gray-900 rounded-t-lg">
            <h3 class="text-sm font-semibold text-white">Joint Assignment - Diaphragm</h3>
        ...
        <div class="p-4 text-sm text-gray-200">
                ...
                <div class="max-h-52 overflow-auto">
                    <template x-for="opt in options" :key="opt.id">
                        <label class="flex items-center gap-2 px-2 py-1.5 rounded cursor-pointer hover:bg-gray-700"
                               :class="selectedId === opt.id ? 'bg-blue-900/50' : ''">
                            <input type="radio" name="jd-option" :value="opt.id" x-model="selectedId" class="accent-blue-500">
            ...
            <div class="mt-2 text-[11px] text-gray-400">
                "From Area": el nudo hereda el diafragma de la losa que lo cubre.
                "Disconnect": el nudo queda fuera de cualquier diafragma.
            </div>
            <div class="mt-1 text-[11px] text-gray-400 text-right" x-text="count + ' nodo(s) seleccionado(s)'"></div>
        </div>
...
<script>
    function jointDiaphragmsModal() {
        return {
            open: false,
            count: 0,
            options: [],
            selectedId: "FROM_AREA",

            init() {
                window.addEventListener('open-joint-diaphragms-modal', (e) => {
                    this.options = e.detail?.options || [];
                    this.selectedId = e.detail?.currentId || 'FROM_AREA';
            ...
            apply() {
                window.cadSystem?.applyJointDiaphragmFromModal?.(this.selectedId);
            },
