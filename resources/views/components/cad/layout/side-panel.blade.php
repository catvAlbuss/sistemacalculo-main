...
<aside class="cad-bg cad-border flex h-full basis-1/4 flex-col border-r-4">
  <x-cad.panel title="Items" init="isOpen = false">
      ...
      <li x-data="{ open: false }">
        <div class="collapsible m-1 flex items-center rounded-sm p-2 text-xs hover:bg-gray-300">
          <svg class="collapse-icon h-4 w-4 transition-transform duration-200" @click="open = ! open"
            :class="open ? '' : '-rotate-90'" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
            stroke="currentColor">
    ...
    <ul>
      <li x-data="{ open: false }">
        <div class="collapsible m-1 flex items-center rounded-sm p-2 text-xs hover:bg-gray-300" @click="open = ! open">
          <svg class="collapse-icon h-4 w-4 transition-transform duration-200" :class="open ? '' : '-rotate-90'"
      ...
      <div class="flex flex-col gap-2 p-2">
        ...
        <x-cad.panel-properties title="Fuerza">
          ...
          <x-cad.input-properties label="Multiplicador"
            ...
            handleInput=""></x-cad.input-properties>
          <x-cad.input-properties label="Fx" bind="moveObjectState.nodeX" handleInput=""></x-cad.input-properties>
          <x-cad.input-properties label="Fy" bind="moveObjectState.nodeY" handleInput=""></x-cad.input-properties>
        </x-cad.panel-properties>
    ...
    <template x-if="currentState === selectedBeamsState && selectedBeamsState.selectedObjects.length > 1">
      <div class="p-2" x-data="{
        ...
        <x-cad.panel-properties title="Material">
          ...
          <x-cad.input-properties label="Área de la sección" bind="currentState.selectedObjects[0].A" handleInput=""
            disabled="true"></x-cad.input-properties>
        </x-cad.panel-properties>
      </div>
    </template>
      ...
      <div class="p-2">
        ...
        <x-cad.panel-properties title="Material">
          <x-cad.input-properties label="Modulo Elástico" bind="selectedBeamsState.selectedObjects[0].E"
            handleInput=""></x-cad.input-properties>
          <x-cad.seccion-select bind="selectedBeamsState.selectedObjects[0].A" handleInput=""></x-cad.seccion-select>
    ...
    <template
      ...
      <div class="p-2">
        <x-cad.panel-properties title="Parametros">
          <x-cad.input-properties label="Longitud" bind="selectedParametricState.selectedObjects[0].longitud"
          ...
          <x-cad.input-properties label="#Fuerzas" bind="selectedParametricState.selectedObjects[0].forceAmount"
          ...
          <x-cad.input-properties label="Fy"
            ...
            handleInput="selectedParametricState.selectedObjects[0].changeForces(selectedParametricState.currentLoad)"></x-cad.input-properties>
          <div class="flex-row">
            <x-primary-button @click="addToScene(selectedParametricState.selectedObjects[0])">Añadir</x-primary-button>
            <x-secondary-button
    ...
    <template x-if="currentState === editParametricState && currentState.editing">
      <div class="p-2">
        <x-cad.panel-properties title="Material">
          ...
          <x-cad.input-properties label="Área de la Sección" bind="currentState.editing[0].A" handleInput=""
            disabled="true"></x-cad.input-properties>
        </x-cad.panel-properties>
      </div>
    </template>
