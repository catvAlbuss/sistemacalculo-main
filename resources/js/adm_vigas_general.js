...
$(document).ready(function () {
  ...
  document.getElementById("accionButton").addEventListener("click", (e) => {
    ...
    for (let i = 0; i < tramos * 3; i++) {
      ...
      var a = (
        d -
        Math.sqrt(Math.pow(d, 2) - (2 * Math.abs(momentosUltimos[i] * Math.pow(10, 5))) / (0.9 * 0.85 * fc * bases[i]))
      ).toFixed(2);
      als.push(a);
      //refuerzo calculado
    ...
    for (let i = 0; i < tramos * 3; i++) {
      ...
      var vucortante =
        (parseFloat(replicatedResults[i]) + parseFloat(Mn_Derresult[i])) / luzLibres + cargascapacidadcmcv;
