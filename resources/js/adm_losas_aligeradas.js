$(document).ready(function () {
        ...
        for (let i = tds.length; i < numLuz; i++) {
          ...
          if (tr.querySelector("th").textContent.includes("Luz libre (m)")) {
            ...
            input.classList.add(
              ...
              "rounded-md",
              "text-center",
            ); // Agregar clase para reducir el tamaño
  ...
  function resultadosAceroA1(columna = 1, value = 0.28, num_tramos = 1, cantidad = 1) {
    ...
    var a = parseFloat(
      (d - Math.sqrt(Math.pow(d, 2) - (2 * Math.abs(mu_i * Math.pow(10, 5))) / (0.9 * 0.85 * base))).toFixed(2),
    );
  ...
  function resultadosAceroB1(columna = 1, value = 0.28, num_tramos = 1, cantidad = 1) {
    ...
    var d1 = altura - 3;
    var mn1 = (0.9 * (0.85 * fc * base * aReal1) * (d1 - aReal1 / 2)) / 100000;
    ...
    a1 = parseFloat(
      (d1 - Math.sqrt(Math.pow(d1, 2) - (2 * Math.abs(mu_usar * Math.pow(10, 5))) / (0.9 * 0.85 * fc * base))).toFixed(
        2,
      ),
    );
    /* console.log("a1: " + a1) */
