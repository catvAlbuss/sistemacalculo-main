...
export function createSimpleAxisLabel(text, color, scene, position = null) {
  try {
...
export function createAxisLabel3D(text, color, scene) {
  try {
    ...
    const plane = BABYLON.MeshBuilder.CreatePlane(`label3D_${text}`, { width: 0.6, height: 0.6 }, scene);
...
export function createNumberLabel(text, x, y, z, scene) {
  try {
    ...
    const material = createLabelMaterial(`numMat_${text}_${x}_${y}_${z}`, texture, new BABYLON.Color3(1, 1, 1), scene);

    const plane = BABYLON.MeshBuilder.CreatePlane(`num_${text}_${x}_${y}_${z}`, { width: 0.35, height: 0.35 }, scene);
...
export function createTextPlane(text, position, scene) {
  try {
    const texture = new BABYLON.DynamicTexture(`textTexture_${text}`, { width: 128, height: 64 }, scene, true);
    ...
    const plane = BABYLON.MeshBuilder.CreatePlane(`textPlane_${text}`, { width: 0.8, height: 0.4 }, scene);
...
export function createAxisWithNumbers(scene, name, color, x, y, z) {
  try {
    ...
    const axis = BABYLON.MeshBuilder.CreateLines(`${name}Axis`, { points: [start, end] }, scene);
...
// NUEVAS METODOS PARA LOS LABELS

export function createStoryLabel3D(text, position, scene, color = null) {
  try {
    // Usar un color por defecto si no se proporciona
    const textColor = color || new BABYLON.Color3(1, 1, 0.8); // Amarillo claro

    // Crear textura más simple
    const texture = new BABYLON.DynamicTexture(
      `story_label_${Date.now()}_${Math.random()}`,
    ...
    // Dividir texto en líneas si tiene \n
    ...
    console.error(`Error creando label de piso ${text}:`, error);
    return null;
  }
}

export function createElevationLabel3D(text, position, scene, color = null) {
  try {
    const textColor = color || new BABYLON.Color3(1, 0.8, 0.2); // Amarillo más visible
    
    const texture = new BABYLON.DynamicTexture(
      `elev_label_${Date.now()}_${Math.random()}`,
    ...
    console.error(`Error creando label de elevación ${text}:`, error);
    return null;
  }
}
