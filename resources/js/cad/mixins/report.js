 ...
 * @mixin reportMixin
 *
 * Generación de reportes del modelo y los resultados de análisis.
 *
 * Compila la información del modelo (nodos, barras, cargas, reacciones,
 * desplazamientos) en un documento PDF usando pdfMake (disponible como
 * window.pdfMake por la configuración de Vite).
 *
 * También gestiona el diálogo de "Nuevo Modelo" (openNewModelDialog),
 * que aunque parece de UI está aquí porque comparte la lógica de
 * serialización con los reportes.
 *
 * Responsabilidades:
 * - generarReporte()          → compila y descarga el reporte en PDF
 * - openNewModelDialog()      → diálogo de creación de nuevo modelo
 * - buildReportNodes()        → tabla de nodos para el reporte
 * - buildReportFrames()       → tabla de barras para el reporte
 * - buildReportReactions()    → tabla de reacciones de apoyo
 * - buildReportDisplacements() → tabla de desplazamientos nodales
 */
export const reportMixin = {
  generarReporte() {
    ...
    pdfMake.createPdf(docDefinition).download("Resultados_ETABS.pdf");
  },
