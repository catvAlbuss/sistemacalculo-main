...
export default () => ({
  ...
  //   const nextId = Math.max(maxShapeId + 1, nextShapeCandidate || 1, nextBeamCandidate || 1);
  ...
  //   const isDifferentZFrame = dz > tol;
  //   const isSpatialInclinedFrame = dz > tol && (dx > tol || dy > tol);

  //   const isVertical3DFrame = dz > tol && dx <= tol && dy <= tol;
  ...
  //     isCrossPlaneFrame: options.createdAcrossWorkPlanes === true || isSpatialInclinedFrame || isVertical3DFrame,
  ...
  //   this.showMessage?.(`Barra 3D creada entre nodo ${node1.id} y nodo ${node2.id}.`);
